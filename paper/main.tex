...
Under fixed-pool intent-to-treat PAR10 (capped solver-time cost), at least one
structure-aware variant is below Base in each Gurobi class and tolerance, whereas no robust
runtime gain appears under CPLEX. In exploratory single-default-seed direct Gurobi controls
(endpoint hierarchy selected after inspection), the coverage effect is endpoint- and
class-dependent. Measured as preprocessing-plus-solver time, the implemented role-blind
(Flat) pipeline is faster in Simple and SG-Ter-Mer at both tolerances---a margin dominated
by the selective rule's own preprocessing overhead rather than a pure effect of scaling
more rows---while in the $27$ eligible Full pairs at $0.1\%$ MIPGap the selective augmented
implementation is faster end-to-end, a direction that does not survive a worst-case
sensitivity over four unrecorded instances. On capped solver time the effects are small
(medians at most $1.6\%$): the augmented Simple contrast is the only comparison with
nominal adjusted values below $0.05$ under the reported multiplicity and
operational-family analyses, the deterministic Work comparison reproduces its direction,
and the common-pool strict/mixed reliability-adjusted PAR10 ordering instead favors the
selective rule there. The coefficient-only solver-time contrasts are inconclusive. External
...
\begin{itemize}
  ...
  role-blind pipeline is faster in Simple and SG-Ter-Mer at both tolerances (a margin
  driven largely by the selective rule's own preprocessing cost, and worth tens of
...
overhead is invisible at that resolution; only the dedicated batches support end-to-end
...
treating either as a preregistered confirmatory outcome. The name is deliberately narrow:
...
over repetitions. \Cref{subsec:stage-attribution} reports the outcome: the direction
replicates on every instance, while the absolute gap is load-dependent. The deterministic Gurobi Work comparison and the
...
after inspecting these data and is exploratory). On that endpoint the outcome is one-sided
at both tolerances in the two fast classes: the \emph{implemented Flat preprocessing
pipeline} is faster end-to-end in Simple and SG-Ter-Mer under both kernels, with median
effects of $2.1$--$5.3\%$ and adjusted values below $0.05$ in all eight contrasts (worst
$q_{12}^{\mathrm{blk}}=0.031$). This end-to-end difference combines the numerical coverage
policy with implementation-specific preprocessing overhead and should not be interpreted
as a pure causal effect of scaling more rows: the selective implementation pays roughly
three times the Flat scan for role classification, scale diagnosis, and the coupling grid
search, and on instances that solve in a few seconds that fixed margin is decisive. A
generator that already maintains the row taxonomy could amortize much of it; no
cached-metadata rerun is available in these batches.
...
over all $306$ instances of the three classes. The ordering is not an artifact of the
execution order: the selective rule's preprocessing median exceeds the Flat median on
\emph{every one} of the $306$ instances in both kernels ($204/204$ Simple, $68/68$
SG-Ter-Mer, $34/34$ Full; paired Wilcoxon $p\le1.2\times10^{-10}$ in each cell). The
\emph{magnitude}, however, is protocol-dependent. Under the replication's concurrent load the
median selective-to-Flat ratio is $5.3$--$7.1\times$ (medians $0.34$ versus $0.057$\,s in
Simple, $0.19$ versus $0.031$\,s in SG-Ter-Mer, $0.54$ versus $0.11$\,s in Full), whereas the
dedicated batches recorded $2.7$--$3.1\times$ on smaller absolute values; the Flat calls are
also far noisier in relative terms (median coefficient of variation $0.78$--$1.30$ versus
$0.23$--$0.32$ for the selective calls), as expected of a shorter measurement. We therefore
read the direction of the preprocessing gap as established and its absolute size as specific
to the machine and load under which it is measured.

The absolute magnitudes matter for how much this is worth
(\cref{tab:flat-absolute}). In Simple the median paired end-to-end margin is
...
instance}, and they are decisive only because these runs are themselves short. The
decomposition also refines the attribution: in Simple the preprocessing term accounts for
essentially the whole margin ($0.051$ of $0.067$\,s at $0.1\%$), whereas in SG-Ter-Mer the
solver term contributes about half ($0.038$ of $0.070$\,s), so ``preprocessing-driven'' is
accurate for Simple but only partly so for SG-Ter-Mer. Full is the opposite regime: the
preprocessing difference is the same $0.08$\,s, but the solver term is $-13.4$\,s, three
orders of magnitude larger, which is why its sign reverses. In Full the direction reverses: in the $27$ eligible pairs at $0.1\%$ the
...
Hodges--Lehmann estimate, or the reliability-adjusted aggregate costs. The corresponding
adversarial location estimates still favor selectivity in direction (median SA/Flat
$0.960$, Hodges--Lehmann $0.947$), so what the sensitivity removes is the rank-based
evidence, not the sign of the point estimate. The three double timeouts enter as
...
augmented kernel closes it without metadata; the implemented Flat pipeline is faster
end-to-end at both tolerances, while the solver-time contrast favors Flat at $0.1\%$ only
...
selectivity in the $27$ eligible pairs at $0.1\%$, but it does not survive the worst-case
missing-pair sensitivity, and the solver-time direction clears only the within-tolerance
adjustment. Exporting its matrices
...
eligible pairs and is not retained by the adversarial signed-rank sensitivity); on the solver-time
...
\begin{table}[t]
\centering
\caption{Absolute paired magnitudes behind the ratios of \cref{tab:flat-control}: medians of
$\Delta T=T^{\rm SA}-T^{\rm Flat}$ over the jointly completed pairs, in seconds. A positive
value favors Flat. The end-to-end margins in the two fast classes are tens of milliseconds
per instance; in Full the solver term dominates and reverses the sign.}
...
selective rule's end-to-end paired advantage does not persist in the aggregate readings:
...
$1\%$, $30/26$ at $0.1\%$, and $1/0$ in SG-Ter-Mer at both tolerances), so the
reliability-adjusted costs differ by at most a few seconds and no reversal of the kind seen
in Simple-Augmented occurs; complete values are in Table~S9. No reversal here is a paired
...
search performance. The exploratory direct SA--Flat controls add a further separation,
stated in full in \cref{subsec:stage-attribution}: end-to-end, the implemented Flat
pipeline has the lower observed time in the two fast classes---by tens of milliseconds per
instance, carried mostly (Simple) or partly (SG-Ter-Mer) by the selective rule's
implementation-specific preprocessing overhead---while the Full selectivity direction is
conditional on its eligible pairs; on capped solver time only augmented Simple at $0.1\%$
remains below $0.05$ after every reported multiplicity and operational-family sensitivity
applied to the capped solver-time endpoint, while the strict/mixed \PARten{} audit
reverses that Simple aggregate ordering toward SA-Aug
...
answer it endpoint by endpoint: end-to-end, the implemented Flat pipeline wins the two
fast classes at both tolerances, the Full selectivity direction is conditional on its
eligible pairs, and on solver time alone only the augmented Simple contrast at $0.1\%$
remains below $0.05$ after every adjustment, with all matrix-only solver-time contrasts
inconclusive. Third,
...
end-to-end ordering (\cref{subsec:stage-attribution})---though in absolute terms that
...
than for any single run. The timeout evidence in this study
...
as future work. The end-to-end endpoint is built from preprocessing calls that the dedicated
...
all $306$ instances but also shows its absolute size to be load-dependent, so end-to-end
...
depends on what one measures---end-to-end, where the selective rule pays for its own
diagnosis, against solver time alone, where the effects are small and the
...
preprocessing, which on fast instances can decide the end-to-end ordering.
