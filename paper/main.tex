...
endpoint---it charges each rule the cost of its own intervention---and capped solver time
as a decomposition of that endpoint, isolating the subsequent solver response, without
treating either as a preregistered confirmatory outcome. Strictly, the observed
preprocessing-plus-solver time is an \emph{implementation} endpoint: our prototype rebuilds
structural metadata that the proposed architecture assumes the generator already holds, so a
cached-metadata sensitivity (\cref{tab:flat-cached}) is required to approximate the intended
generator-integrated deployment, and we report both. The name is deliberately narrow:
...
fixed execution order, without repetition; per-policy medians and IQRs are in Table~S10
...
unrelated instances (full protocol and per-cell statistics in Table~S10; dispersion is
...
and pair-specific attrition after exclusions are reported per contrast in Table~S10. For
...
SG-Ter-Mer contrast below $0.05$ on the prototype preprocessing-plus-solver endpoint, while
the augmented Full contrast at $0.1\%$ is conditional on its eligible pairs
(\cref{subsec:stage-attribution}). We therefore call the
...
depend on the relative speed of the two environments. Because the cached effects sit close to
one, we examine sensitivity to a common multiplicative transfer factor,
$T^{\mathrm{cached}}(\lambda)=T_{\rm solver}+\lambda\,T^{\mathrm{cached}}_{\rm preproc}$.
This varies the overall weight of preprocessing relative to solver time, but it does not model
policy-specific hardware or compiler effects: the two rules execute different code, so a change
of machine or toolchain need not rescale them by the same factor. The qualitative conclusion is stable under a common factor: over the evaluated grid
$\lambda\in\{0.25,0.5,1,2,4,8\}$ the cached effects remain close to parity and only a small
minority of fast-class contrasts falls below the nominal adjusted threshold (one to three of
eight); the same two Simple contrasts are retained over $\lambda\in[0.5,8]$, and no
SG-Ter-Mer contrast becomes significant at any grid point (its median ratio rounds to
$1.016$ throughout, its preprocessing term being negligible against solver time).

That common-factor reading is, however, the optimistic one. Allowing the two policies to
transfer differently---sweeping the ratio
$\rho=\lambda_{\rm SA}/\lambda_{\rm Flat}$ with $\lambda_{\rm Flat}=1$---shows the
conclusion is sensitive to that asymmetry in one direction. For $\rho\in[0.5,2]$ the picture
is unchanged (two to five of eight fast-class contrasts below the threshold), but at
$\rho=2$ the augmented SG-Ter-Mer contrasts also cross it and at $\rho=4$ seven of eight do,
while at $\rho=0.25$ the coefficient-only Simple median falls below one and the ordering
reverses. In other words, the finding that removing metadata reconstruction moves the effects
close to parity is robust to how heavily preprocessing is weighted overall, but the exact
count of retained contrasts depends on an unmeasured policy-specific transfer ratio. Measuring
that ratio requires running the cached microbenchmark on the solve hardware itself, which we
have not done; the release-build replication below bounds one component of it. The effect is substantial. The median ratios collapse towards parity---in Simple
from $1.029$ to $1.001$ at $1\%$ and from $1.032$ to $1.010$ at $0.1\%$, in SG-Ter-Mer from
$1.053$ to $1.007$ and from $1.047$ to $1.016$---and, of the eight fast-class contrasts whose
prototype preprocessing-plus-solver ordering had adjusted values below $0.05$, six keep a
direction favoring the role-blind rule but only one retains a within-tolerance nominal
adjusted value below $0.05$: the augmented Simple contrast at $0.1\%$ ($q_{6}=0.010$), which
also survives the joint and operational-family corrections ($q_{12}=0.021$,
$q_{12}^{\mathrm{blk}}=0.032$) and is small in median magnitude, about $0.9\%$. It indicates a
systematic directional difference in this campaign, not a large operational speedup. The
augmented SG-Ter-Mer contrasts fall to $q_{6}=0.115$ and $0.052$, and every coefficient-only
fast-class contrast to $q_{6}\ge0.115$.

The build matters here, and measurably. Our campaigns were compiled without optimization---the
production executable uses the same \texttt{-std=c++17 -g} flags as the instrumented one---so
we repeated the pinned microbenchmark with a release build
...
ratio essentially unchanged ($5.4$--$6.1\times$ against $5.1$--$6.0\times$), but it does move
the cached ratios towards parity, and enough to change one conclusion: under the diagnostic
build the coefficient-only Simple contrast at $1\%$ had $q_{6}=0.0002$, whereas under the
release build it falls to $q_{6}=0.115$. This is also the one policy-specific transfer factor
we can measure rather than assume. Taking the toolchain change as a realized perturbation, the
implied ratio $\rho=\lambda_{\rm SA}/\lambda_{\rm Flat}$ lies in $[0.94,1.01]$ across the six
class--kernel cells---well inside the band in which the qualitative conclusion is stable---so
the asymmetry the sweep warns about is small for the one hardware-or-toolchain change we
actually performed.
The Full augmented direction is unchanged in sign and strength
($0.877$, $q_{6}=0.045$ at $0.1\%$), but it rests on the same $27$ eligible pairs and is
likewise not retained by the adversarial signed-rank sensitivity over the four unrecorded
instances ($p=0.43$). Read together with the phase breakdown, the reading is therefore that
most of the magnitude and apparent uniformity of the fast-class role-blind advantage comes
from metadata reconstruction in the prototype; a smaller, class- and kernel-dependent
residual coverage effect remains after that stage is removed.


Because that margin rests on preprocessing calls that the dedicated batches timed once, in a
fixed order, we re-timed the preprocessing call alone under a replicated, order-randomized
protocol (\cref{subsec:statistical}): $R=20$ repetitions per instance and kernel, order drawn
from a seeded RNG within each repetition, every call in a fresh process, the MIP never solved,
over all $306$ instances of the three classes. The direction of the preprocessing-cost
difference is reproduced under randomized execution order: the selective rule's
preprocessing median exceeds the Flat median on \emph{every one} of the $306$ instances in
both kernels ($204/204$ Simple, $68/68$ SG-Ter-Mer, $34/34$ Full), and, aggregating to the
operational families used elsewhere for dependence, all $34$ Simple families, all $34$
SG-Ter-Mer families and all $34$ Full singletons have a positive median
selective-minus-Flat difference in both kernels. The combined preprocessing-plus-solver
comparisons of \cref{tab:flat-control} remain based on the original fixed-order solve
batches; this replication validates the preprocessing-overhead direction, not the combined
endpoint. Because a concurrent replication is a stress test rather than a clean estimator of
a millisecond-scale quantity, we repeated it in a low-contention protocol---one worker,
every call pinned to a single logical CPU, otherwise identical. That protocol is far
quieter: the relative interquartile range falls to $0.00$--$0.02$ from $0.18$--$1.83$, and
absolute times fall about fivefold. The direction is unchanged---again $306/306$ instances
in both kernels---so the sign survives both a loaded and a quiet machine, while the
magnitude does not: the median paired selective-to-Flat ratio is $5.1$--$6.0\times$ pinned
against $6.1$--$9.1\times$ under load, and $2.7$--$3.1\times$ on the smaller absolute values
of the dedicated batches. Pinned medians are $0.049$ against $0.008$\,s in Simple, $0.030$
against $0.006$\,s in SG-Ter-Mer and $0.077$ against $0.014$\,s in Full (per-cell values,
dispersion and family counts in Table~S10). We therefore read the direction of the
preprocessing gap as established, and use the pinned protocol as the cleaner reference
estimate for this implementation and hardware environment rather than as a universal
intrinsic size.
...
the same log-ratio variable; negative $r_{\mathrm{rb}}$ favors Flat; $q_6$ and $q_{12}$ are
the BH-adjusted value within the six-test tolerance family. The joint twelve-test value
$q_{12}$ and the operational-family sensitivities are reported in the accompanying text and in
Table~S11. The analysis conditions on the per-instance median
...
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}llc r rrr@{}}
\toprule
Gap & Class & Kernel & $n$ & med.\ SA/Flat & $r_{\mathrm{rb}}$ & $q_{6}$ \\
\midrule
\multirow{6}{*}{$1\%$}
 & Simple & Aug & 204 & 1.0004 & $-0.090$ & 0.318 \\
 & Simple & Mat & 204 & 1.0011 & $-0.167$ & 0.115 \\
 & SG-Ter-Mer & Aug & 67 & 1.0075 & $-0.303$ & 0.115 \\
 & SG-Ter-Mer & Mat & 67 & 0.9976 & $-0.140$ & 0.318 \\
 & Full & Aug & 31 & 0.9193 & $+0.355$ & 0.173 \\
 & Full & Mat & 32 & 0.9866 & $+0.246$ & 0.318 \\
\midrule
\multirow{6}{*}{$0.1\%$}
 & Simple & Aug & 204 & 1.0094 & $-0.253$ & 0.010 \\
 & Simple & Mat & 203 & 0.9981 & $+0.089$ & 0.410 \\
 & SG-Ter-Mer & Aug & 67 & 1.0159 & $-0.313$ & 0.052 \\
 & SG-Ter-Mer & Mat & 67 & 1.0008 & $-0.109$ & 0.526 \\
 & Full & Aug & 27 & 0.8771 & $+0.529$ & 0.045 \\
 & Full & Mat & 27 & 0.9803 & $+0.053$ & 0.822 \\
\bottomrule
\end{tabular}
\end{table}
...
shrinks the gap against the role-blind rule by nearly an order of magnitude, on the estimated
cached-metadata implementation cost rather than on any hardware-independent intrinsic cost. An
...
as future work. The prototype preprocessing-plus-solver endpoint is built from calls that the dedicated
...
cached-metadata endpoint and hardware-transfer sensitivity (Table~S11).
