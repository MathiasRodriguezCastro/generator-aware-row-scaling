...
(Flat) pipeline has lower observed time in Simple and SG-Ter-Mer at both tolerances---a
margin driven mainly by implementation-specific preprocessing overhead in Simple and by a
mixture of preprocessing overhead and solver response in SG-Ter-Mer, rather than by a pure
effect of scaling more rows---while in the $27$ eligible Full pairs at $0.1\%$ MIPGap the
selective augmented implementation has lower observed preprocessing-plus-solver time, a
direction that is not retained by an adversarial signed-rank sensitivity over four
unrecorded instances. On capped solver time the effects are small
...
($\le0.6$\,ms). Charging only the intrinsic work---the cached-metadata counterpart
$T_{\mathrm{preproc}}^{\mathrm{cached}}=T_{\mathrm{diagnosis}}+T_{\mathrm{application}}$,
which is what a generator that maintains the taxonomy would pay---collapses the gap from
...
it was given, but a small residual cost of selectivity survives that correction.
...
\begin{table}[t]
\centering
\caption{Completed-pair component summaries associated with the comparisons of
\cref{tab:flat-control}: medians of $\Delta T=T^{\rm SA}-T^{\rm Flat}$, in seconds. These
