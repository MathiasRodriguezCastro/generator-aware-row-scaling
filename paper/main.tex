...
uniformly superior. Runtime effects are of the order of the solver's own unreplicated
run-to-run variability, which a single-run-per-cell design cannot separate from them; only one
of twelve contrasts clears multiplicity adjustment, and none under CPLEX. Under the
...
\begin{itemize}
  ...
  the solver's own unreplicated run-to-run variability.
...
the original comparison never ran. We ran it as a third arm on ClusterUY under Gurobi. On the
cells completed so far---Simple at $1\%$ ($202$ matched instances) and SG-Ter-Mer at both
tolerances ($54$ each)---the metadata-routed rule \emph{matches} the role-blind rule on capped
solver time: the paired median $T_{\mathrm{full}}/T_{\mathrm{Flat}}$ is between $0.997$ and
$1.001$ in every cell and kernel, with no robust difference (Simple $1\%$ augmented $1.0000$,
$p=0.88$; matrix-only $1.0008$, $p=0.17$; SG-Ter-Mer $0.1\%$ augmented $1.0000$, $p=0.71$).
Covering the residual rows does not itself move runtime relative to the selective rule
($T_{\mathrm{full}}/T_{\mathrm{selective}}$ not significant in any cell). This sharpens the
reading of the coverage result: it is the \emph{coverage gap}---the highest-magnitude rows the
selective taxonomy declined to scale---not the use of role metadata, that separated the
selective rule from full coverage. Metadata routes rows; it is not the lever, and it need not
be an exclusive filter. (Simple $0.1\%$ and the Full class were interrupted by a login-node
outage and are re-running; the ordering above is what the completed cells show.)
