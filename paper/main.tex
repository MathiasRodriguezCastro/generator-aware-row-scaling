...
cached-metadata sensitivity only the augmented Simple contrast at $0.1\%$ remains below the
reported joint and operational-family thresholds and the augmented Full contrast stays
...
cleaner pinned diagnostic protocol produces smaller absolute times, and the release-build
values used in \cref{tab:flat-cached} are reported separately in Table~S12. It does
not, however, reverse the ordering: the role-blind kernel remains marginally cheaper in all
six class--kernel cells under every protocol, and under the pinned release build---the
sensitivity of record---it has the lower cached preprocessing time on $585$ of the $612$
instance--kernel records ($607/612$ pinned diagnostic, $408/612$ in the noisier concurrent
protocol; \texttt{results-revision/preproc-timing/}). That
