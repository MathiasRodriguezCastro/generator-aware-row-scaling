...
\abstract[Abstract]{%
Generated mixed-integer programs (MIPs) reach the solver as flat coefficient matrices,
although the generator knows which rows belong to each component and which rows couple
components. We ask whether this pre-export metadata can usefully drive positive row
scaling---a transformation that multiplies complete constraints by strictly positive factors
and therefore preserves the mathematical program exactly, while numerical feasibility of
returned solutions is checked in the original scale. We propose a structure-aware layer that
applies geometric-mean row normalization in a local--block--coupling order and evaluate it on a
hydrothermal-dispatch generator across three instance classes, two commercial solvers, and two
tolerances. The operational family is compared with unscaled, Ruiz-kernel, and solver-internal
baselines; direct Gurobi role-blind controls apply either the augmented or coefficient-only
kernel to every row. The structure-aware variants generally lower own-incumbent path-specific
LP diagnostics relative to the unscaled and Ruiz-kernel formulations. Separately, the synthetic
exported-range analysis finds no general advantage over full-coverage per-row normalization.
Under fixed-pool intent-to-treat PAR10, at least one structure-aware variant is below Base in
each Gurobi class and tolerance, whereas no robust runtime gain appears under CPLEX. In the
single-default-seed direct Gurobi controls, the coverage effect is endpoint- and
class-dependent. Measured as total pipeline time---the primary endpoint, which charges each
rule its own preprocessing---full coverage is faster in Simple and SG-Ter-Mer at both
tolerances, whereas the selective augmented rule is faster end-to-end in Full at $0.1\%$
MIPGap. On capped solver time at $0.1\%$ the effects are small (medians at most $1.6\%$):
only the augmented Simple contrast survives joint multiplicity and operational-family
sensitivities, the deterministic Work comparison reproduces its direction, and the
common-pool strict/mixed reliability-adjusted PAR10 ordering instead favors the selective
rule there. The coefficient-only solver-time contrasts are inconclusive. External
preprocessing is solver-dependent, and cross-solver persistence of the coverage ordering
remains untested.
}
...
\begin{itemize}
  ...
  \item In the reported single-default-seed Gurobi control batches, coverage versus
  selectivity depends on the endpoint. End-to-end (total pipeline time), full coverage is
  faster in Simple and SG-Ter-Mer at both tolerances and the selective rule is faster in
  Full at $0.1\%$, all retained after adjustment. On capped solver time the medians are at
  most $1.6\%$ and only the augmented Simple contrast at $0.1\%$ survives every
  sensitivity; all coefficient-only comparisons remain inconclusive.
...
$\varepsilon$; a factor below the floor leaves the row at base scale. \methodAug{}-LF is off
by default and is evaluated separately at
$\varepsilon_{\mathrm{floor}}^{\mathrm{loc}}=10^{-2}$. Its condition on $\alpha_r$ is not
invariant to a positive re-emission of a row; here it is keyed to the generator's canonical
physical-unit export, the same reference used by the original-scale audit. The caveat,
invariant alternatives, and sensitivity sweep are reported in Table~S4. Neither floor is a
general guarantee, and the primary \methodAug{} results deliberately retain the unresolved
local-reliability behavior rather than masking it.
...
The direct SA--Flat analysis has its own endpoints and multiplicity structure. Total
pipeline time $T_{\mathrm{total}}=T_{\mathrm{preproc}}+T_{\mathrm{solver}}$ is the
\emph{primary} operational endpoint, because it includes the cost of the intervention
itself; capped solver time is the secondary endpoint used to isolate the subsequent solver
response, and the deterministic Gurobi Work comparison is a replication check. Here
$T_{\mathrm{preproc}}$ is the wall time of the preprocessing call on the constructed
model---row-role classification, scale diagnosis (including the coupling-stage grid search
where applicable), and in-place application of the factors---and excludes model
construction, solver model loading, and post-solve verification, which are common to both
policies. For the time endpoints the primary paired variable is
$d_i=\log(T_{\mathrm{Flat},i}/T_{\mathrm{SA},i})$: the Wilcoxon signed-rank test, the
rank-biserial correlation, the operational-family sign-flip sensitivity, and the family
bootstrap are all computed from this same relative-scale variable, so the tested estimand
coincides with the reported median ratio; negative values favor full coverage. (Work, a
deterministic count that can be zero, keeps the paired difference.) The primary convention
defines each MIPGap tolerance as a distinct six-test family (three classes by two kernels),
because changing the stopping tolerance changes the operating regime; this is an analysis
convention rather than an externally preregistered choice, so we also report the joint BH
adjustment over all twelve contrasts. Each endpoint forms its own BH families, and the
secondary endpoints add no confirmatory claims beyond the primary one. Both readings
initially treat instances as independent. As a sensitivity under family-level joint sign
...
instance-level $p$-values may understate uncertainty. On the secondary solver-time endpoint
the family-sign sensitivity retains only the augmented Simple contrast below $0.05$ after
adjustment; on the primary total-time endpoint it retains every Simple and SG-Ter-Mer
contrast and the augmented Full contrast at $0.1\%$. We therefore call the SG-Ter-Mer
solver-time result nominal, read the tests as evidence about scenarios of
...
run costs $36{,}000$\,s. The class separation below does not depend on the mixed-reading
...
pairs, rather than comparing two separate speedups against Base. Total pipeline time is the
primary endpoint (\cref{subsec:statistical}); it answers the operational question because the
selective rule must pay for its own diagnosis. On that endpoint the outcome is one-sided at
both tolerances in the two fast classes: full coverage is faster end-to-end in Simple and
SG-Ter-Mer under both kernels, with median total-time effects of $2.1$--$5.3\%$, and every one
of those eight contrasts is retained by the within-tolerance and joint adjustments and by the
operational-family sensitivity (worst retained value
$q_{12}^{\mathrm{tot,block}}=0.031$). Most of that margin is the selective rule's own
preprocessing overhead, which dominates in these fast cells. In Full the direction reverses:
at $0.1\%$ the selective augmented rule is faster end-to-end (median ratio $0.878$) and
survives all adjustments ($q_{12}^{\mathrm{tot}}=0.025$,
$q_{12}^{\mathrm{tot,block}}=0.028$), while the same direction at $1\%$ is inconclusive. On
the secondary capped solver-time endpoint---which isolates the solver's response to the
exported matrix---no $1\%$ contrast survives adjustment; at $0.1\%$ the augmented Simple
contrast favors full coverage and survives every adjustment ($q_{6}=0.010$, $q_{12}=0.021$,
$q_{12}^{\mathrm{block}}=0.032$), the augmented SG-Ter-Mer contrast is nominal ($p=0.026$,
$q_{6}=0.052$), and the augmented Full contrast favors selectivity within tolerance
($q_{6}=0.045$) but not jointly ($q_{12}=0.090$). The solver-time effects are
small---median $0.9\%$ and $1.6\%$ in Simple and SG-Ter-Mer, with a family-bootstrap
interval that includes one in SG-Ter-Mer---but the deterministic Work-unit comparison
reproduces both augmented $0.1\%$ solver-time directions
($q_{6}^{\mathrm{block}}=0.036$ and $0.006$; Table~S10), so the sixteen-thread
parallel-trajectory concern does not explain them. None of the six matrix-only solver-time
contrasts is conclusive.
The Full direction remains a small, heavily censored single-campaign observation, and the
Flat controls were not run under CPLEX, so cross-solver persistence of this ordering is
untested.
...
augmented kernel closes it without metadata; its end-to-end contrast favors Flat at both
tolerances, while the solver-time contrast favors Flat at $0.1\%$ only nominally. The
coefficient-only solver-time contrast remains neutral.
...
\cref{subsec:synthetic-results}.) In Full the augmented contrast favors selectivity: the
end-to-end advantage at $0.1\%$ survives all adjustments, while the solver-time direction
clears only the within-tolerance adjustment. Exporting its matrices
...
effect. After the joint multiplicity and operational-family sensitivities, the retained
orderings are: on the primary total-time endpoint, full coverage in Simple and SG-Ter-Mer
(both kernels and tolerances) and selectivity in Full at $0.1\%$; on the secondary capped
solver-time endpoint, only the augmented $0.1\%$ Simple contrast---and its strict/mixed
...
kernels. Each row compares the structure-aware (SA) and role-blind Flat reruns from the same
dedicated class--tolerance batch on two endpoints: the primary total pipeline time
$T_{\rm total}=T_{\rm preproc}+T_{\rm solver}$ and the secondary capped solver time
(\cref{subsec:statistical}). For each endpoint the paired variable is the log time ratio
$d_i=\log(T_{\mathrm{Flat},i}/T_{\mathrm{SA},i})$, so the test, the effect size, and the
reported median ratio share one estimand; negative $r_{\mathrm{rb}}$ favors Flat.}
...
\toprule
& & & & \multicolumn{3}{c}{Total time (primary)} & \multicolumn{3}{c}{Solver time (secondary)} \\
\cmidrule(lr){5-7}\cmidrule(lr){8-10}
Gap & Class & Kernel & $n$ &
med. $\frac{T_{\rm total}^{\rm SA}}{T_{\rm total}^{\rm Flat}}$ & $r_{\mathrm{rb}}$ & $q_{6}$ &
...
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item A one-sided timeout is censored at $T_{\max}=3600$\,s; a one-sided operational abort is
assigned the same cost by convention, as in the Base comparisons (the $10T_{\max}$
sensitivity is identical because these direct pairs contain no one-sided aborts). Double
non-completions are excluded; no exact-zero differences occur. Tests are two-sided Wilcoxon
on $d_i$ with the implementation of \cref{subsec:statistical}. Here $q_6$ applies BH within
tolerance and per endpoint (the primary, not externally preregistered convention); the
joint $q_{12}$, the operational-family sign-flip values, and the family-bootstrap
intervals are in Table~S10 and quoted in the text where they change a conclusion.
\item $T_{\rm preproc}$ is the wall time of the preprocessing call itself
(\cref{subsec:statistical}); for a noncompletion the total combines it with the capped
solver cost. Preprocessing is small in absolute terms but non-negligible in these fast
cells, and it is the dominant part of the total-time margin in Simple and SG-Ter-Mer.
\item Attrition from the nominal pools ($204$/$68$/$34$): Simple loses no pair at $1\%$
and one double noncompletion at $0.1\%$ (Matrix-only, $n=203$); each SG-Ter-Mer contrast
loses one double noncompletion ($n=67$); Full at $1\%$ loses three (Augmented, $n=31$) and
two (Matrix-only, $n=32$) double noncompletions; at Full $0.1\%$ the batch recorded $30$ of
$34$ instances (045--048 absent) and three double noncompletions leave $n=27$. The full
per-contrast decomposition is in Table~S10. The analysis is over eligible capped-time
pairs, not a post-selected set of jointly optimal runs; all comparisons are reproducible
with \texttt{scripts/flat\_control\_tests.py}.
\item Each of the six dedicated CSVs contains both SA and Flat reruns from the same
class--tolerance batch; these batches are independent of the main experiment. Each
instance--variant cell is one run at the default solver seed, so the paired tests measure
variation across operational instances, not seed-to-seed variability. The complete
strict/mixed reliability audit is in Table~S9; the central readings are in
\cref{tab:flat-reliability-body}.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
\begin{table}[t]
\centering
\caption{Common-pool reliability readings for the two central augmented SA--Flat contrasts
at $0.1\%$ MIPGap (Gurobi). \PARten{} in seconds under the intent-to-treat, strict, and
mixed readings of \cref{subsec:original-scale-audit}, with the completed-run failure counts
...
Because the reliability-adjusted audit changes the Simple conclusion, its central numbers
belong in the body (\cref{tab:flat-reliability-body}). In Simple, Flat-Aug wins
...
Flat-Aug retains a small descriptive advantage under all three readings. Neither reversal
is a paired test; it is the same fixed-pool aggregate convention as
...
search performance. The direct SA--Flat controls add a further separation. End-to-end, full
coverage is faster in Simple and SG-Ter-Mer at both tolerances---largely the selective
rule's own preprocessing overhead---while selectivity is faster in Full at $0.1\%$. On
capped solver time at $0.1\%$ only the augmented Simple contrast survives every
sensitivity, and the strict/mixed \PARten{} audit reverses that Simple aggregate ordering
toward SA-Aug; SG-Ter-Mer retains only a small descriptive Flat-Aug advantage
(\cref{tab:flat-reliability-body}; Tables~S9--S10).
...
answer it endpoint by endpoint: end-to-end, full coverage wins Simple and SG-Ter-Mer at
both tolerances and selectivity wins Full at $0.1\%$; on solver time alone, only the
augmented Simple contrast at $0.1\%$ survives every adjustment, the SG-Ter-Mer contrast is
nominal, the Full contrast clears only the within-tolerance adjustment, and every
matrix-only solver-time contrast is inconclusive. Third,
whether the solver could have done this itself: the internal-scaling control
(\cref{subsec:internal-scaling}) gives a class-dependent answer. On Simple, the selected
internal mode matches the external rule on the typical paired instance despite an aggregate-tail
difference. On SG-Ter-Mer, the external rule retains a sizable descriptive paired shift. Full
is directionally favorable at $1\%$, whereas at $0.1\%$ tuned internal modes overtake it. The
metadata's value is therefore attributed strictly to the second comparison, not to the Ruiz
contrast, which concerns the kernel alone.
...
comparison favors Flat in the two $0.1\%$ Simple/SG cells (the latter nominal) and favors
selectivity in Full within tolerance, while the matrix-only solver-time contrasts are all
inconclusive. The structure-aware rules do lower the
...
the main scenario, the internal-scaling baseline, and the coupling-stage activation campaign
...
retains Simple but not SG-Ter-Mer after adjustment; the total-time and Work endpoints do
retain SG-Ter-Mer (Table~S10). The parallel-trajectory concern is partly answered by the
deterministic Work replication of both augmented $0.1\%$ directions, but seed-replicated
runs are left as future work.
...
(\cref{tab:flat-control}) show that coverage and selectivity cannot be generalized across the
two kernels or across endpoints. In the reported single-default-seed Gurobi campaign, the
primary end-to-end endpoint is one-sided in the two fast classes: measured as total pipeline
time, full coverage is faster in Simple and SG-Ter-Mer under both kernels and both tolerances
(median effects $2$--$5\%$, retained by every adjustment), largely because the selective rule
pays for its own diagnosis, whereas in Full at $0.1\%$ the selective augmented rule is faster
end-to-end and survives all adjustments. On the secondary capped solver-time endpoint the
effects are small (medians $0.9\%$ and $1.6\%$): only the augmented Simple contrast at
$0.1\%$ survives every sensitivity---and strict/mixed \PARten{} reverses that Simple
aggregate ordering toward SA-Aug---while the SG-Ter-Mer contrast is nominal, the Full
contrast favors selectivity within tolerance only, and every coefficient-only solver-time
contrast is inconclusive. These controls were run only under Gurobi:
the value of external preprocessing is solver-dependent, but whether the same
...
opposite runtime direction in Full (its solver-time contrast clears only the
within-tolerance adjustment, so the solver-dynamics reading remains a hypothesis, although
the end-to-end reversal at $0.1\%$ survives all adjustments in this single campaign). The
reported SG-Ter-Mer campaign shows
that a selective taxonomy can miss important rows. The Full campaign raises the
complementary hypothesis that blanket normalization can
overshoot on integrated models. A practical scaling layer should therefore measure both
coverage policies, exactly as the role-blind control does here.
...
operational-family, bootstrap, total-time, Work-unit, and own-incumbent-diagnostic
