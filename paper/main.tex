...
Supporting Information; the augmented exploratory variant differs only by folding the right-hand
side into the per-row and per-block magnitude extremes.
...
carry forward, in preference to the augmented rule, whose use of the right-hand side introduces an
additional documented original-scale reliability failure mode (the matrix-only rule still requires
the same original-scale verification); the augmented rule is better read as an exploratory probe of
...
Only once the operational policies are compared using a native exhaustive role assignment and
identical coverage can any remaining difference be attributed to the use of generator metadata;
the synthetic comparison already meets that bar, which is why it can make the attribution.
