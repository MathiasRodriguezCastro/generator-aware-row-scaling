...
layer under study applies geometric-mean row normalization in a local--block--coupling order
while preserving the MIP exactly; returned solutions are verified on the original constraints.
Its coupling stage never activates on any operational instance tested, so the operational
evidence covers the local and block stages only and coupling remains an unvalidated
extension. We
...
controls. The structure-aware variants are associated with lower own-incumbent fixed-LP
diagnostics---a readout taken along each variant's own solve path, and so not a clean measure
of representation quality---but a synthetic mechanism check finds no general exported-range
advantage over applying the same per-row kernel to every row, and a direct spectral audit of
that grid finds no conditioning advantage either. Under fixed-pool intent-to-treat
PAR10---an endpoint selected after inspecting the data, hence exploratory---some
structure-aware variant falls below Base in each Gurobi class and tolerance, though not the
same variant across cells and without replicating across tolerances; no robust gain appears
under CPLEX. Direct exploratory Gurobi controls show that the coverage ordering
depends on class, kernel, and endpoint. Capped solver-time effects are small---of the order of
this solver's unreplicated run-to-run variability, which the single-run-per-cell design cannot
separate from them---and only the augmented Simple contrast at $0.1\%$ has nominal adjusted
values below $0.05$ across the reported multiplicity and operational-family analyses, while a
secondary reliability-adjusted PAR10 reading of that same cell points the same way without
adding independent support. The current prototype has lower preprocessing-plus-solver
...
sensitivity on a release build moves the effects toward parity and, under the baseline
cross-protocol construction, leaves one of the eight fast-class contrasts with a
within-tolerance nominal adjusted value below $0.05$. In Full, the selective direction is not supported by the adversarial signed-rank
...
\begin{itemize}
  ...
  \item Positive per-row factors lowered the reported own-incumbent path-specific fixed-LP
  diagnostic without changing the MIP, though that path-specific readout is not a clean measure
  of representation quality. The stage-attribution ablation traces essentially all of that
  movement to the metadata-free local stage, and the coupling stage never activated on any
  operational instance tested, so it remains operationally unvalidated; on an already
  well-scaled model the selective rule can widen the exported range instead.
  ...
  disappears: six directional orderings remain, but on a release build---under the baseline
  cross-protocol construction---only one contrast (Simple, augmented, $0.1\%$; about $0.9\%$
  in median) keeps a within-tolerance nominal adjusted value below $0.05$. On capped solver time the medians
  ...
  \item Under fixed-pool intent-to-treat PAR10---an exploratory endpoint selected after
  inspecting the data---some structure-aware variant falls below Base in each Gurobi class and
  tolerance, but not the same variant in each, and the ordering does not persist across
  tolerances. SG-Ter-Mer at $0.1\%$ is the only cell with both
...
whose stages, coverage, and pipeline sensitivity are evaluated---and partly refuted---empirically.
...
over applying the same kernel to every row.
...
row-multiplicative imbalance is present. Their use as a rationale for \emph{selectivity} does
not: the role-blind control matches or beats the augmented structure-aware rule on
$\kappa_2$ in every pattern, including the coupling patterns the coupling stage was designed
for. The spectral argument therefore supports scaling, not metadata.
...
profiles, and primal--dual progress; and the sensitivity of the observed preprocessing effect
to the tested solver pipeline through separate Gurobi and CPLEX
...
and most consequential for the claims of this paper, \methodAug{} obtains no spectral advantage
over its role-blind control. Their median ratio $\kappa_2^{+}(\methodAug{})/\kappa_2^{+}
(Flat)$ at $S=6$ is $1.00$ on None, Local and Coupling~(uniform); on the two patterns
where the coupling disturbance is the dominant one it is $2.36$ (Coupling) and $10.37$ (Mixed),
both against \methodAug{}. On the very design case that motivates the coupling stage, treating
every row as local is spectrally better than using the metadata to treat coupling rows
selectively. This is an independent confirmation, in a quantity the operational campaigns never
measure, of the coverage-versus-selectivity result of
\cref{subsec:added-controls,subsec:stage-attribution}.
...
cleaner pinned diagnostic protocol produces smaller absolute times, and the release-build absolute preprocessing values used in \cref{tab:flat-cached} are reported in
...
contrasts fall to $q_{6}=0.115$ and $0.051$, and every coefficient-only fast-class contrast
...
optimization---the executable used in the original solver campaigns was compiled with the same
\texttt{-std=c++17 -g} flags as the instrumented diagnostic build---so we repeated the pinned microbenchmark with a release build
($-\mathtt{O2}$ $-\mathtt{DNDEBUG}$) and take it as the implementation-cost sensitivity of
record. Optimization shrinks every preprocessing time about sixfold and leaves the prototype preprocessing-call
...
build it falls to $q_{6}=0.115$. Splicing release preprocessing onto diagnostic-build solver times is only legitimate if
optimization changes the \emph{cost} of preprocessing without changing the transformation it
produces, so we verified that directly over every available record: exporting the preprocessed
model with both builds for all $306$ instances, both kernels and both coverage policies gives
$1204$ byte-identical exports out of $1224$
(\texttt{scripts/build\_invariance\_audit.py}). Every Simple and SG-Ter-Mer record is
byte-identical across builds. These are the two fast classes underlying the eight cached
contrasts summarized in the abstract and the only retained adjusted result; for them the build
change affects preprocessing time but not the exported model, so the comparison is a clean
cost substitution. The $20$ exceptions are five Full instances (029, 035, 045, 047, 048), each
differing under all four variants. The differing entries are the nonlinear-hydro linearization
coefficients, and they differ by one unit in the last place ($4.7\times10^{-16}$ relative, in
$720$ of $47{,}173$ coefficients on a representative instance); the audit therefore localizes
the discrepancy to optimization-dependent floating-point evaluation during model construction
rather than to a change in the selected scaling factors or safeguard decisions. Two of the
five (029, 035) fall inside the analysed Full pools, so the Full cached rows recombine a
release preprocessing time with a solver time recorded on a model that differs at the last
bit. This build comparison provides one observed
policy-specific
...
particular compiler change, while the cross-environment transfer asymmetry itself remains unmeasured.
...
toolchain need not rescale them by the same factor. On the evaluated common-factor grid
$\lambda\in\{0.25,0.5,1,2,4,8\}$, one fast-class contrast has $q_{6}<0.05$ for
$\lambda\le2$, and two do at the evaluated points $\lambda=4$ and $8$; the augmented Simple
contrast at $0.1\%$ has $q_{6}<0.05$ at every evaluated grid point. On the policy-specific
grid $\rho=\lambda_{\rm SA}/\lambda_{\rm Flat}\in\{0.25,0.5,1,2,4\}$ with
$\lambda_{\rm Flat}=1$, the corresponding counts are one for $\rho\le1$, three at $\rho=2$
and five at $\rho=4$. Additional $q_{6}$ crossings therefore first appear at the evaluated
point $\rho=2$, but the transition threshold between $1$ and $2$ is not identified by this
grid. These counts concern within-tolerance $q_{6}$ only and do not imply retention under the
joint or operational-family adjustments. The $\lambda$ and $\rho$ grids are exploratory
log-spaced stress ranges, chosen to vary the relative preprocessing contribution by factors of
two over a deliberately broad interval; they are not confidence regions or calibrated
estimates of the workstation-to-ClusterUY transfer. The contrasts crossing at each grid point
are identified in Table~S12. Complete contrast-by-grid values are provided in
\texttt{results-revision/preproc-timing/cached\_transfer\_sensitivity.csv} in the
...
coupling-uniform pattern; on the spectral reading of that same grid it does not beat role-blind
coverage (\cref{subsec:spectral-audit}). The coupling
...
an audit in Table~S11. All Simple and SG-Ter-Mer exported models are byte-identical across
the diagnostic and release builds. For Full, five instances exhibit last-bit
model-construction differences across builds, including two that enter the analysed pools;
the four Full rows (marked $\dagger$) are therefore approximate cross-build timing
sensitivities, whereas the central fast-class comparison changes preprocessing cost only.
Median ratios are SA/Flat on
...
lower observed time in the two fast classes at both tolerances; under the baseline cross-protocol construction of the
cached-metadata sensitivity, which better approximates generator-integrated deployment, the
effects move close to parity and only one fast-class contrast retains $q_{6}<0.05$; the number
of within-tolerance crossings changes under the hypothetical transfer-factor stress grids of
Table~S12. The Full selectivity
...
runtime on real instances where it activates. That marginal mechanism is exercised only on the
synthetic benchmark, and there the evidence is unfavorable rather than merely incomplete: on
the magnitude-range proxy its distinctive repair is confined to the oversized branch of its
collective design case, and on the direct spectral reading it does not beat role-blind
coverage. Transferring it to another operational generator remains open.
...
collective design case. The spectral audit of that same grid (\cref{subsec:spectral-audit})
measures $\kappa_2^{+}$ directly, and its verdict is split: every rule cuts the
pseudo-condition number by seven to nine orders of magnitude wherever row-multiplicative
imbalance is present, but \methodAug{} obtains no advantage over its role-blind control,
tying on three patterns and losing on the two coupling-dominated ones. This supports no
claim of spectral optimality or of a conditioning advantage of metadata over full
coverage. On the
...
alone. Under the baseline cross-protocol construction, removing metadata reconstruction moves the
fast-class effects close to parity: six of
...
(Figures~S1--S4) and twelve tables---hyperparameter sensitivity (Table~S1), the per-variant
...
transfer-factor sensitivity (Table~S12).
