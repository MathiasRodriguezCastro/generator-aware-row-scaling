...
identical, so for the two fast classes---where the cached comparison is decided---the builds
differ in speed alone, not in row factors, scaled rows, or safeguard decisions. The $20$
exceptions are five Full instances (029, 035, 045, 047, 048), each differing under all four
variants, which locates the discrepancy in model construction rather than in scaling: the
differing entries are the nonlinear-hydro linearization coefficients, and they differ by one
unit in the last place ($4.7\times10^{-16}$ relative, in $720$ of $47{,}173$ coefficients on
a representative instance), as expected when optimization changes floating-point contraction.
Two of the five (029, 035) fall inside the analysed Full pools; for those the release
preprocessing time corresponds to a model differing from the archived one only at the last
...
joint or operational-family adjustments.
...
nonlinear-hydro coefficients under optimization, so for those the release preprocessing time
is measured on a model that differs from the archived one at the level of floating-point
rounding. The two fast classes, which decide the cached comparison, are exactly invariant. The prototype preprocessing-plus-solver endpoint
