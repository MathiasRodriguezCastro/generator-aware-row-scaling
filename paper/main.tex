...
(medians at most $1.6\%$): after adjustment only the augmented Simple contrast remains
below $0.05$, the deterministic Work comparison reproduces its direction, and the
...
\begin{itemize}
  ...
  dominated by the selective rule's own preprocessing cost); the Full selectivity
  direction is conditional on the $27$ eligible pairs. On capped solver time the medians
  are at most $1.6\%$ and only the augmented Simple contrast at $0.1\%$ remains below
  $0.05$ after every sensitivity; all coefficient-only comparisons remain inconclusive.
...
grid search where applicable), and in-place application of the factors. Each call is timed
once with the C++ high-resolution wall clock in the batch's fixed execution order; calls
were neither interleaved in randomized order nor repeated, so the end-to-end inference is
specific to this implementation and timing protocol. The measured distributions are
nonetheless tight and well separated (per-policy medians and IQRs in Table~S10: medians
$0.02$--$0.13$\,s, IQRs $\le0.003$\,s). The deterministic Gurobi Work comparison and the
...
as a pure causal effect of scaling more rows: the absolute preprocessing costs are tiny
and tightly concentrated (per-policy medians $0.02$--$0.13$\,s, IQRs $\le0.003$\,s;
Table~S10), but the selective implementation pays roughly three times the Flat scan for
role classification, scale diagnosis, and the coupling grid search, and on instances that
solve in a few seconds that fixed margin dominates. A generator that already maintains the
row taxonomy could amortize much of it; no cached-metadata rerun is available in these
batches. In Full the direction reverses: in the $27$ eligible pairs at $0.1\%$ the
selective augmented implementation is faster end-to-end (median ratio $0.878$,
$q_{12}^{\mathrm{pre+solve}}=0.025$), but the result is conditional on those pairs. Four
nominal instances (045--048) were never recorded, and a worst-case missing-pair
sensitivity---appending them with adverse signs at above-maximal ranks, because
multiplicity adjustment does not address missing-data selection---does not retain the
contrast ($p=0.47$); the three double timeouts enter as operational ties. At $1\%$ the
...
eligible pairs and fails the worst-case missing-pair sensitivity); on the solver-time
...
class--tolerance batch, on the exploratory endpoint hierarchy of \cref{subsec:statistical}.
...
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item A one-sided timeout is censored at $T_{\max}=3600$\,s; a one-sided abort receives the
same cost by convention. Double non-completions are excluded; no exact-zero differences
occur. Each cell is one run at the default solver seed. The end-to-end margins combine the
coverage policy with implementation-specific preprocessing overhead
(\cref{subsec:stage-attribution}).
\item The Full $0.1\%$ rows use the $27$ eligible pairs of the $30$ recorded instances;
the significant augmented contrast does not survive the worst-case sensitivity over the
four unrecorded instances (\cref{subsec:stage-attribution}).
\item Joint $q_{12}$, sign-flip and bootstrap sensitivities, Hodges--Lehmann estimates,
per-contrast attrition, preprocessing-time distributions, the abort sensitivity, and the
batch protocol are in Table~S10; the strict/mixed reliability audit is in Table~S9 and
...
direction as established. No reversal here is a paired test; all use the fixed-pool
aggregate convention of \cref{tab:par10-readings}.
...
pipeline is faster in the two fast classes---largely the selective rule's
...
remains below $0.05$ after every sensitivity, and the strict/mixed \PARten{} audit
...
end-to-end ordering (\cref{subsec:stage-attribution}). The timeout evidence in this study
...
as future work. The end-to-end inference additionally rests on single, unrandomized,
unrepeated preprocessing timings (protocol and per-policy distributions in
\cref{subsec:statistical} and Table~S10), and the adjusted Full result is conditional on
the $27$ eligible observed pairs: the worst-case missing-pair sensitivity over instances
...
two kernels or across endpoints; the full statement is in \cref{subsec:stage-attribution}.
In brief: in these exploratory single-default-seed Gurobi controls, the implemented Flat
preprocessing pipeline is faster end-to-end in the two fast classes (median $2$--$5\%$,
dominated by the selective rule's own preprocessing overhead rather than a pure effect of
scaling more rows), while the Full selectivity direction is conditional on the $27$
eligible pairs and fails a worst-case missing-pair sensitivity; on the solver-time
decomposition the effects are small and only augmented Simple at $0.1\%$ remains below
$0.05$, with the strict/mixed \PARten{} audit reversing that aggregate ordering toward
SA-Aug. These controls were run only under Gurobi:
...
and fails the worst-case missing-pair sensitivity, so the solver-dynamics reading remains
