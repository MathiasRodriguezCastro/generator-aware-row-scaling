...
$q_{6}<0.05$. In Full, the selective point
estimate is not retained under an adversarial missing-pair sensitivity. The observed effect
...
pairs, rather than comparing two separate speedups against Base. Preprocessing-plus-solver
time is the operational endpoint (\cref{subsec:statistical}; the hierarchy was selected
after inspecting these data and is exploratory). On that endpoint the observed ordering is
...
(\cref{tab:flat-cached}). This sensitivity splices the per-instance cached preprocessing
times of the low-contention replication onto the solver times of the original batches, so
its preprocessing component is measured under the cleanest protocol available rather than
under load. The effect is substantial. The median ratios collapse towards parity---in Simple
...
$q_{12}^{\mathrm{blk}}=0.008$ and $0.013$). The augmented SG-Ter-Mer contrasts fall to
...
the BH value within the six-test tolerance family; the joint $q_{12}$ and operational-family
sensitivities are quoted in the text. The analysis conditions on the per-instance median
...
\toprule
& & & & \multicolumn{3}{c}{Pre+solve time (operational)} & \multicolumn{3}{c}{Solver time (decomposition)} \\
...
alone, where the effects are small and the
reliability-adjusted reading can point the other way. They were run only under Gurobi:
the value of external preprocessing is solver-dependent, and whether the same
coverage-versus-selectivity ordering persists under CPLEX remains untested. Establishing this
bounded result
...
and CPLEX distributional, diagnostic, and performance-profile views (Figures~S1--S4) and ten
tables. They report hyperparameter sensitivity (S1), the per-variant reliability and
\PARten{} audit (S2), the coupling- and local-floor checks (S3--S4), uniformly unavailable
instances and full-pool sensitivity (S5), paired-test attrition (S6), the available
internal-scaling sweep and verifier audit (S7), activation-audit runtime replicates (S8),
the direct SA--Flat original-scale reliability audit (S9), and the multiplicity,
operational-family, bootstrap, preprocessing-plus-solver, Work-unit, and
own-incumbent-diagnostic
sensitivities for the direct controls, with their per-contrast attrition (S10). Additional
processed logs and rebuild scripts are available in the reproducibility repository.
