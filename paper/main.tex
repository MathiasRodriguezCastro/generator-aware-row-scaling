...
while preserving the MIP exactly. We verify every returned solution against the original
unscaled constraints, and under the pre-declared absolute criterion the proposed default
\emph{degrades} that reliability relative to the unscaled base, failing more often on
zero-right-hand-side balance rows; we report the strict reading as primary rather than a
more permissive one introduced afterwards.
...
local rows and coupling rows \emph{might} warrant different treatment. Whether that distinction
improves conditioning is an empirical question, and the direct audit of
\cref{subsec:spectral-audit} answers it in the negative: every scaling rule improves $\kappa_2$
by orders of magnitude, but role-differentiation obtains no conditioning advantage over its
role-blind control---and none even on the coupling-dominated patterns it is meant to address.
...
The augmented rule is specified as a three-stage structure-aware procedure that first rescales
local rows, then summarizes the resulting local block scales, and finally uses those block
scales to treat coupling rows separately. The second stage is a diagnosis rather than a
transformation: the block scales are never applied to the blocks, and enter only the coupling
proxy. On the operational classes the third stage is inert as well---the activation audit of
\cref{subsec:stage-attribution} records zero activations and byte-identical full and local-only
exports on every eligible pair---so what the solver actually receives there is a row-by-row
normalization restricted to the role-selected local rows. The coupling stage activates only on
the oversized synthetic coupling patterns of \cref{subsec:synthetic-results}, and its operational
contribution is therefore untested.
...
\noindent \noindent \Cref{tab:robust-gurobi-01pct-mipgap} shows that the same descriptive completed-run
...
stage. It fires only when the coupling proxy $\widehat\rho$ leaves the near-unity band, and in
this testbed it does not because the only coupling rows are demand and reserve balances whose
coefficients on the generation variables are near unity: $\widehat\rho\approx3\times10^{-4}$ in
Simple and exactly $1$ in SG-Ter-Mer and Full. That is a fact about which coupling rows this
dispatch formulation has, and in particular about its lack of a transmission network. A model
