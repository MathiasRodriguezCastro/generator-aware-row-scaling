...
attains an order-of-magnitude lower condition number than a role-blind rule with the \emph{same}
kernel and coverage ($30/30$ instances, $p<10^{-8}$), demonstrating that the mechanism can produce
a material conditioning gain. The operational campaigns never enter that regime, where a per-row
...
ordering. Separately, on the synthetic benchmark the spectral evidence is split by structural rule. The
augmented collective rule obtains no advantage over its role-blind counterpart and is worse on
the coupling-dominated patterns, so the full-coverage Flat rule matches or beats it on both the
magnitude-range proxy and $\kappa_2^{+}$. The kernel-matched heterogeneous-block experiment
isolates the opposite result for the matrix-only rule: under identical coverage and the same
coefficient-only kernel, \methodMat{} reduces $\kappa_2^{+}$ by a median factor of $10.2$
relative to Flat-Mat once the blocks sit at heterogeneous scales (\cref{subsec:spectral-audit}).
The evidence therefore supports the block-relative placement of the coefficient-only kernel in
the regime designed to exercise it---a conditioning gain of the metadata over a same-kernel
role-blind rule---but not the augmented collective coupling rule, and it makes no claim of
spectral optimality. On the
...
role-blind rule (\cref{subsec:spectral-audit}), so the experiment demonstrates that the mechanism
can produce a material conditioning gain in a controlled heterogeneous-block regime; what remains
is to connect that conditioning gain to runtime and to build an operational testbed whose coupling
rows enter the same regime, which the present one---with its near-unity balance rows---does not. For deployment in the meantime the safe rule keeps the structure-aware order but
