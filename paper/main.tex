...
SG-Ter-Mer contrast below $0.05$, while the augmented Full contrast at $0.1\%$ is
...
($\le0.6$\,ms). Charging only the intrinsic work---an estimated cached-metadata counterpart
...
one, we check that dependence directly by evaluating
$T^{\mathrm{cached}}(\lambda)=T_{\rm solver}+\lambda\,T^{\mathrm{cached}}_{\rm preproc}$
over a wide band of hardware-transfer factors. The conclusion is stable: across
$\lambda\in[0.25,8]$---a $32$-fold range---the number of fast-class contrasts with
$q_{6}<0.05$ stays between one and three of eight, the same two Simple contrasts carry it
for $\lambda\in[0.5,8]$, and no SG-Ter-Mer contrast becomes significant at any $\lambda$
(its median ratio is unchanged at $1.016$ throughout, its preprocessing term being negligible
against solver time). The effect is substantial. The median ratios collapse towards parity---in Simple
...
Table~S10. The analysis conditions on the per-instance median
...
$c^{(k,\mathrm{solver})}$ (recomputing with the preprocessing-plus-solver cost shifts
every cell by less than $0.2$\,s and changes no ordering); completed-run failure counts
...
shrinks the gap against the role-blind rule by nearly an order of magnitude. An
...
as future work. The preprocessing-plus-solver endpoint is built from calls that the dedicated
...
(Figures~S1--S4) and ten tables---hyperparameter sensitivity (Table~S1), the per-variant
...
direct controls, with their per-contrast attrition and timing environment (Table~S10).
