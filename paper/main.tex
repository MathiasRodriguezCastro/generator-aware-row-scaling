...
($607/612$ pinned diagnostic, $408/612$ in the noisier concurrent protocol; computed by
\texttt{scripts/preproc\_timing\_replication.py} from the \texttt{cached\_sa\_s} and
\texttt{cached\_flat\_s} columns of, respectively,
\path{results-revision/preproc-timing/replicated_pinned_release.csv},
\path{results-revision/preproc-timing/replicated_pinned.csv} and
\path{results-revision/preproc-timing/replicated.csv}). These record-level counts
condition on the $R=20$ per-instance median estimates: they measure the direction of the
estimated medians and do not propagate within-instance timing uncertainty. To screen that
noise we classify each record by comparing its paired difference against the dispersion
measured inside the same instance, calling it a \emph{within-dispersion tie} when
$\lvert\widetilde T_{\mathrm{SA},i}-\widetilde T_{\mathrm{Flat},i}\rvert\le B_i$, with
$B_i$ the larger of the two policies' interquartile ranges over the $R=20$ repetitions
(Table~S12). This is a descriptive noise screen, not a confidence interval, an equivalence
test, or a domain-calibrated practical-significance margin. The classification gives $461$
...
under the concurrent protocol, whose count is close to a coin flip precisely because almost
every record there is measurement-indeterminate. That
