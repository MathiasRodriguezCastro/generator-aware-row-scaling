...
sensitivity moves the effects toward parity and leaves only two of the eight contrasts with
within-tolerance nominal adjusted values below $0.05$. In Full, the selective direction is not supported by the adversarial signed-rank
...
\begin{itemize}
  ...
  disappears: six directional orderings remain, but only two contrasts keep within-tolerance
  nominal adjusted values below $0.05$. On capped solver time the medians
...
direction favoring the role-blind rule but only two retain within-tolerance nominal adjusted
values below $0.05$: the coefficient-only Simple contrast at $1\%$ ($q_{6}=0.0002$) and the
augmented Simple contrast at $0.1\%$ ($q_{6}=0.008$). Those two also survive the joint and
operational-family corrections ($q_{12}=0.0005$ and $0.008$;
$q_{12}^{\mathrm{blk}}=0.008$ and $0.013$). Both are nonetheless small in median magnitude:
about $0.2\%$ for Simple coefficient-only at $1\%$ MIPGap and $1.0\%$ for Simple augmented at
$0.1\%$. They indicate systematic directional differences in this campaign, not large
operational speedups. The augmented SG-Ter-Mer contrasts fall to
$q_{6}=0.094$ and $0.051$, and both coefficient-only SG-Ter-Mer contrasts to $q_{6}\ge0.29$.
...
which removes the prototype's metadata-reconstruction stage. Median ratios are SA/Flat on
...
\midrule
\multirow{6}{*}{$1\%$}
 & Simple & Aug & 204 & 1.0008 & $-0.124$ & 0.185 \\
 & Simple & Mat & 204 & 1.0019 & $-0.333$ & 0.0002 \\
 & SG-Ter-Mer & Aug & 67 & 1.0074 & $-0.303$ & 0.094 \\
 & SG-Ter-Mer & Mat & 67 & 0.9976 & $-0.148$ & 0.291 \\
 & Full & Aug & 31 & 0.9197 & $+0.355$ & 0.173 \\
 & Full & Mat & 32 & 0.9869 & $+0.242$ & 0.286 \\
\midrule
\multirow{6}{*}{$0.1\%$}
 & Simple & Aug & 204 & 1.0096 & $-0.259$ & 0.008 \\
 & Simple & Mat & 203 & 0.9983 & $+0.065$ & 0.506 \\
 & SG-Ter-Mer & Aug & 67 & 1.0159 & $-0.314$ & 0.051 \\
 & SG-Ter-Mer & Mat & 67 & 1.0036 & $-0.116$ & 0.506 \\
 & Full & Aug & 27 & 0.8771 & $+0.529$ & 0.045 \\
 & Full & Mat & 27 & 0.9804 & $+0.053$ & 0.822 \\
\bottomrule
\end{tabular}
\end{table}
...
to parity and only two fast-class contrasts retain $q_{6}<0.05$. The Full selectivity
...
the eight directional orderings remain, but only two small Simple contrasts retain nominal
