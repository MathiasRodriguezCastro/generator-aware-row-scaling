...
equal coverage the two coincide, proved for the matrix-only kernel and confirmed byte-for-byte.
But where the mechanism is exercised the metadata does help: on a synthetic grid with blocks at
heterogeneous scales, the matrix-only structure-aware rule attains an order-of-magnitude lower
condition number than a role-blind rule with the \emph{same} kernel and coverage ($30/30$
instances, $p<10^{-8}$). The operational campaigns never enter that regime, where a per-row local
kernel computable from the flat matrix carries the whole effect. Runtime effects are of the
...
difference is, by \cref{prop:attribution}, attributable to the coupling treatment alone. The
synthetic side of this is already encouraging: on blocks placed at heterogeneous scales the
matrix-only structure-aware rule buys an order of magnitude of conditioning over a kernel-matched
role-blind rule (\cref{subsec:spectral-audit}), so the mechanism is real; what remains is to
connect that conditioning gain to runtime and to build an operational testbed whose coupling rows
enter the same regime, which the present one---with its near-unity balance rows---does not. For deployment in the meantime the safe rule keeps the structure-aware order but
