...
comes from rebuilding metadata the generator already possesses, and a cached-metadata
sensitivity retains only two of those eight orderings. In Full, the selective point
...
\begin{itemize}
  ...
  direction is conditional on the $27$ eligible pairs. On capped solver time the medians
...
resulting spectrum, is given in Appendix~\ref{app:proofs}.
...
singular values, is given in Appendix~\ref{app:proofs}.
...
prototype preprocessing-plus-solver ordering had adjusted values below $0.05$, only two
retain them: the coefficient-only Simple contrast at $1\%$ ($q_{6}=0.0002$) and the augmented
Simple contrast at $0.1\%$ ($q_{6}=0.008$). The augmented SG-Ter-Mer contrasts fall to
...
($0.877$, $q_{6}=0.045$ at $0.1\%$). Read together with the phase breakdown, the operational
reading is therefore that the role-blind advantage observed in the fast classes is mostly an
artifact of what the prototype recomputes, not a property of covering more rows.
...
the BH values within tolerance and jointly. Exploratory, like
...
preprocessing gap as established, and take the pinned protocol as the estimate of its
intrinsic size.
...
class--tolerance batch, on the exploratory endpoint hierarchy of
\cref{subsec:statistical}.
Endpoints: preprocessing-plus-solver time
$T_{\mathrm{pre+solve}}=T_{\rm preproc}+T_{\rm solver}$ (operational) and capped solver
time (decomposition). The paired variable is $d_i=\log(T_{\mathrm{Flat},i}/T_{\mathrm{SA},i})$;
...
answer it endpoint by endpoint: in preprocessing-plus-solver time the implemented Flat
pipeline has the lower observed time in the two fast classes at both tolerances, the Full selectivity direction is conditional on its
eligible pairs, and on solver time alone only the augmented Simple contrast at $0.1\%$
remains below the nominal $0.05$ threshold after every reported multiplicity and
operational-family adjustment applied to the capped solver-time endpoint, with all
...
retains Simple but not SG-Ter-Mer after adjustment; the preprocessing-plus-solver and Work
endpoints do retain SG-Ter-Mer (Table~S10). The Work replication of both augmented
...
depends on what one measures---preprocessing-plus-solver time, where the selective rule pays
for its own diagnosis, against solver time alone, where the effects are small and the
...
\appendix
\section{Proofs of the spectral propositions}
\label{app:proofs}
