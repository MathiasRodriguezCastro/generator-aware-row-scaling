...
The result is controlled but \emph{non-identifying}. The distinctive coupling stage---the only
place an exhaustive role map lets the metadata change the exported matrix---never activates on any
operational instance, so it remains an unvalidated extension. The one substantial difference
between the selective rule and the role-blind full-coverage control traces to an incomplete role
map that left a block of market-shortage rows unscaled, not to the metadata: under equal coverage
the two coincide (proved for the matrix-only kernel and confirmed byte-for-byte), so these
instances neither confirm nor refute the value of the metadata and instead show that a per-row
local kernel, computable from the flat matrix, carries the effect. Runtime effects are of the
...
\begin{itemize}
  ...
  \item Cover every row: the only case where the metadata-selective rule lagged the role-blind
  one was an incomplete role map that left a block of constraints unscaled---close that gap and
  the two coincide. The distinctive coupling stage never fired on any operational instance, so
  these instances do not test the value of the metadata itself.
...
\cref{prop:attribution}, wherever coverage is made equal the structure-aware and role-blind
matrix-only exports are provably---and empirically---identical, so no operational difference can
be pinned on the metadata. Concretely, the paper contributes: (i) an exhaustive-map formulation
...
controls on all three classes under Gurobi, a fixed-basis conditioning control, and a
seed-replicated noise floor---showing that row scaling helps, that the local kernel carries it,
and that these operational instances do not exercise the coupling metadata; and (iv) an explicit
separation of numerical improvement from computational acceleration. \Cref{fig:overview}
...
block and does not enter the local-row normalization stage. There is no third mathematical
class. A row that looks ``global but not coupling'' has simply not been traced to the module
that generates it, so a residual category is a symptom of an incomplete map $\tau$, not a
structural fact of the formulation.
...
than by their own raw norm, is precisely what the role-blind rule cannot do. The gap closes when
the blocks are homogeneous (\methodMat{} and Flat-Mat tie, median ratio $1.02$, on the uniform
family), which is exactly why the operational comparison is non-identifying: its blocks and its
near-unity balance rows do not carry the scale heterogeneity that activates the mechanism. The
benefit is specific to the block-relative \emph{placement} of the per-row kernel and not to the
augmented collective factor---\methodAug{} is an order of magnitude \emph{worse} than Flat-Mat
here ($\times 11$). So the block metadata is not inert: on the numerical diagnostic, in the
regime designed to exercise it, it buys an order of magnitude that a role-blind rule with the
same kernel and coverage cannot. What the operational campaigns show is that the tested
instances do not enter that regime.
...
difference disappears (\cref{subsec:added-controls}); and it states the attribution principle
...
pipelines; the CPLEX flat-control cell we added shows the Flat-versus-selective ordering does not
even hold its sign across solvers at this one-percent scale (\cref{subsec:stage-attribution}). Establishing this
bounded result
...
The present evidence does not establish that generator metadata improves solver performance, but
neither does it refute that hypothesis, and two facts fix the scope. First, the block-relative
coupling stage---the one place \cref{prop:attribution} lets the metadata change the exported
matrix---never activates on any operational instance, so the campaigns never exercise the
distinctive mechanism. Second, the only substantial structure-aware-versus-Flat matrix
difference, in SG-Ter-Mer, came from an incomplete role map that left the market-shortage rows
unassigned, not from the metadata: under equal coverage the two coincide (\cref{prop:attribution},
confirmed byte-for-byte in the matrix-only kernel). The operational campaigns therefore evaluate
local geometric-mean normalization and the consequences of unequal coverage far more directly
than they evaluate block-relative coupling metadata. This is a genuine, if negative,
methodological result---a metadata pipeline with an incomplete taxonomy can be matched by a
role-blind rule that simply reaches every row---but it is a non-identifying result about the
metadata, not a refutation of it.

Two things follow for a next version of the study. Empirically, the structure-aware and
role-blind rules must be compared under an \emph{exhaustive} classification and identical
coverage---every generated row traced to its module or to the coupling set---so that any residual
difference is, by \cref{prop:attribution}, attributable to the coupling treatment alone; and the
synthetic benchmark must include instances that actually drive the coupling stage, with coupling
rows unbalanced relative to already-normalized block scales and equal absolute magnitudes
carrying different numerical meaning by block, since the operational testbed never enters that
regime. For deployment in the meantime the safe rule keeps the structure-aware order but
guarantees coverage: sweep every row with the coefficient-only kernel, applying it only where a
row is poorly scaled and reverting a row to base scale whenever the safeguarded solution fails
original-scale verification. This coverage-safeguarded matrix-only rule is the variant we would
carry forward, in preference to the augmented rule whose use of the right-hand side is what
degrades original-scale reliability; the augmented rule is better read as an exploratory probe of
whether the right-hand side should enter the diagnosis at all. A practical layer should also time
its own preprocessing, which on fast instances can decide the preprocessing-plus-solver ordering.
Only once the policies are compared under exhaustive, identical coverage can any remaining
difference be attributed to the use of generator metadata.
